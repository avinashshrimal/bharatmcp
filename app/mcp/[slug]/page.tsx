...
export function generateStaticParams() {
  return FEATURED_MCPS.map((mcp) => ({ slug: mcp.slug }))
}

export default async function MCPDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params

  // Check if it's an India MCP — redirect to /india/[slug]
  const indiaMcp = INDIA_MCPS.find((m) => m.slug === slug)
  if (indiaMcp) {
    redirect(`/india/${slug}`)
  }

  const mcp = FEATURED_MCPS.find((m) => m.slug === slug)
  if (!mcp) {
    notFound()
  }

  const relatedMCPs = FEATURED_MCPS.filter(
    ...
    <>
      ...
      <main className="relative z-[1] pt-16">
        <div className="max-w-[1200px] mx-auto px-6 py-10">
          ...
          <div className="flex items-start gap-5 mb-8">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap mb-2">
                ...
                <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-[rgba(255,153,51,0.1)] text-[#FFB366] border border-[rgba(255,153,51,0.3)]">
          ...
          <div className="mb-8">
            <h3 className="text-[13px] font-medium text-[var(--color-text-muted)] mb-3 uppercase tracking-wide">Works with</h3>
            <div className="flex flex-wrap gap-2">
              {mcp.compatibility.map((client) => (
                <span
                  key={client}
                  className="inline-flex items-center gap-1.5 text-[12px] font-medium px-3 py-1.5 rounded-full bg-[rgba(255,153,51,0.06)] text-[#FFB366] border border-[rgba(255,153,51,0.2)]"
                >
          ...
          <div className="flex flex-col min-[900px]:flex-row gap-10">
            ...
            <div className="flex-1 min-w-0">
                ...
                <ul className="space-y-3">
                  {mcp.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-3">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#FF9933" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 mt-0.5">
                        <polyline points="20 6 9 17 4 12" />
                      </svg>
              ...
              {/* Setup Steps */}
              <section className="mb-10">
                <h2 className="font-heading font-bold text-[20px] text-white mb-4">Setup Guide</h2>
                <ol className="space-y-4">
                  {mcp.setupSteps.map((step, index) => (
                    <li key={step} className="flex items-start gap-4">
                      <div className="w-7 h-7 rounded-full bg-[rgba(255,153,51,0.1)] border border-[rgba(255,153,51,0.3)] flex items-center justify-center shrink-0">
                        <span className="text-[12px] font-bold text-[#FFB366]">{index + 1}</span>
                      </div>
                      <span className="text-[15px] text-[var(--color-text-secondary)] pt-0.5">{step}</span>
                    </li>
                  ))}
                </ol>
              </section>
            ...
            <aside className="w-full min-[900px]:w-[300px] shrink-0">
              ...
              {finalRelated.length > 0 && (
                <div className="rounded-[14px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
                  <h3 className="font-heading font-bold text-[15px] text-white mb-4">Related MCPs</h3>
                  <div className="flex flex-col gap-3">
                    {finalRelated.map((related) => (
                      <Link
                        key={related.slug}
                        href={`/mcp/${related.slug}`}
